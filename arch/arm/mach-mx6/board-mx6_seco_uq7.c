...
#if defined(CONFIG_Q7_GPIO_MCP23S08)
#include <linux/spi/mcp23s08.h>
#endif
#ifdef CONFIG_Q7_TOUCHSCREEN_ATMEL_MXT
...
#include "board-mx6qd_seco_uq7.h"
#include "board-mx6sdl_seco_uq7.h"

/******************* SD *******************/
//#define MX6_SECO_UQ7_SD1_CD			IMX_GPIO_NR(6, 15)
//#define MX6_SECO_UQ7_SD1_WP			IMX_GPIO_NR(6, 14)
#define MX6_SECO_UQ7_SD4_CD			IMX_GPIO_NR(2, 6)
#define MX6_SECO_UQ7_SD4_WP			IMX_GPIO_NR(5, 20)
/******************* SATA *******************/
#define MX6_SECO_UQ7_SATA_LED		IMX_GPIO_NR(6, 11)
/******************* USB *******************/
#define MX6_SECO_UQ7_USB_OTG_ID		IMX_GPIO_NR(1, 0)
#define MX6_SECO_UQ7_USB_OTG_PWR	IMX_GPIO_NR(3, 22)
#define MX6_SECO_UQ7_USB_HUB_RESET	IMX_GPIO_NR(7, 12)
/******************* PMIC *******************/
#ifdef CONFIG_Q7_PMIC_PFUZE100
#define MX6_SECO_UQ7_PMIC_INT		IMX_GPIO_NR(5, 16)
#endif
/******************* LVDS *******************/
#define MX6_SECO_UQ7_LVDS_BLT_CTRL   IMX_GPIO_NR(1, 4)
#define MX6_SECO_UQ7_LVDS_PPEN_CTRL  IMX_GPIO_NR(1, 2)
/******************* ETHERNET *******************/
#define MX6_SECO_UQ7_FEC_RESET		IMX_GPIO_NR(3, 23)
#define MX6_ENET_125MHz_EN			IMX_GPIO_NR(6, 24)
/******************* SPI *******************/
/******************* REV A/B ***************/
#define MX6_SECO_UQ7_ECSPI1_CS1		IMX_GPIO_NR(3, 19)
#define MX6_SECO_UQ7_ECSPI1_CS2		IMX_GPIO_NR(3, 24)
#define MX6_SECO_UQ7_ECSPI1_CS3		IMX_GPIO_NR(3, 25)
/******************* REV C *****************/
#define MX6_SECO_UQ7_ECSPI2_CS1     IMX_GPIO_NR(5, 12)
#define MX6_SECO_UQ7_ECSPI2_CS2     IMX_GPIO_NR(5, 9)
#define MX6_SECO_UQ7_ECSPI3_CS3     IMX_GPIO_NR(4, 24)
/******************* TSC2006 *******************/
#define MX6_SECO_UQ7_PEN_IRQ        IMX_GPIO_NR(5, 2)
/******************* GPIO EXPANDER *******************/
#if defined(CONFIG_Q7_GPIO_MCP23S08)
#define MX6_SECO_UQ7_GPIO_EX_RST    0   // TO SET  
#define MX6_SECO_UQ7_GPIO_INTA      0   // TO SET  
#define MX6_SECO_UQ7_GPIO_INTB      0   // TO SET
#endif
/******************* MXT1188 ********************/
#ifdef CONFIG_Q7_TOUCHSCREEN_ATMEL_MXT
#define ATMEL_MXT_IRQ         0   // TO SET
#define ATMEL_MXT_RST	      0   // TO SET 
#endif                       
/******************* AUDIO *******************/
...
#endif
/******************* CAN *******************/
#define MX6_SECO_UQ7_CAN_EN          IMX_GPIO_NR(3, 20)
/******************* EMBEDDED CONTROLLER *******************/
#define MX6_SECO_UQ7_ECTRL_IRQ         IMX_GPIO_NR(2, 7)

#define MX6_SECO_UQ7_SD3_WP_PADCFG	(PAD_CTL_PKE | PAD_CTL_PUE |	\
...
//	.cd_gpio = MX6_SECO_UQ7_SD1_CD,
//	.wp_gpio = MX6_SECO_UQ7_SD1_WP,
...
static const struct esdhc_platform_data mx6q_seco_q7_sd4_data __initconst = {
	.cd_gpio = MX6_SECO_UQ7_SD4_CD,
	.wp_gpio = MX6_SECO_UQ7_SD4_WP,
	.keep_power_at_suspend = 1,
};
...
static inline void mx6q_seco_uq7_init_uart(void) {
...
static void imx6q_seco_q7_usbotg_vbus(bool on) {
	if (on)
		gpio_set_value(MX6_SECO_UQ7_USB_OTG_PWR, 1);
	else
		gpio_set_value(MX6_SECO_UQ7_USB_OTG_PWR, 0);
}
...
static void __init imx6q_seco_q7_init_usb(void) {
	 ...
	 */
	ret = gpio_request(MX6_SECO_UQ7_USB_OTG_PWR, "usb-pwr");
	if (ret) {
		pr_err("failed to get GPIO MX6_SECO_UQ7_USB_OTG_PWR: %d\n",
			ret);
		return;
	}
	gpio_direction_output(MX6_SECO_UQ7_USB_OTG_PWR, 0);
...
static void gpio_set_usbotg_init(void) {
	int ret = 0;
	int temp = 0;
	
	ret = gpio_request (MX6_SECO_UQ7_USB_OTG_ID, "USBOTG ID");
	if (ret) {
		printk("failed to get MX6_SECO_UQ7_USB_OTG_ID: %d\n", ret);
	} else {
		temp = gpio_get_value(MX6_SECO_UQ7_USB_OTG_ID);
		printk("USB OTG ID STATUS: %x \n", temp);
		if (cpu_is_mx6q()){	
					
			if(temp == 1)
				mxc_iomux_v3_setup_pad(mx6qd_seco_uq7_otg_id_up_pads);
			if(temp == 0)
				mxc_iomux_v3_setup_pad(mx6qd_seco_uq7_otg_id_down_pads);
		}
		else if (cpu_is_mx6dl()) {
			
			if(temp == 1)
				mxc_iomux_v3_setup_pad(mx6dl_seco_uq7_otg_id_up_pads);
			if(temp == 0)
				mxc_iomux_v3_setup_pad(mx6dl_seco_uq7_otg_id_down_pads);
		}
		
	}
	gpio_free(MX6_SECO_UQ7_USB_OTG_ID);
...
int ret;
        ...
        if (!ret) {
                ...
                gpio_set_value(MX6_SECO_UQ7_FEC_RESET, 0);
                msleep(2);
                gpio_set_value(MX6_SECO_UQ7_FEC_RESET, 1);
                msleep(1);
...
static int mx6q_seco_q7_spi_cs_revA_B[] = {
	MX6_SECO_UQ7_ECSPI1_CS1,
	MX6_SECO_UQ7_ECSPI1_CS2,
	MX6_SECO_UQ7_ECSPI1_CS3,
};
...
static int mx6q_seco_q7_spi_2_cs_revC[] = {
        MX6_SECO_UQ7_ECSPI2_CS1,
	MX6_SECO_UQ7_ECSPI2_CS2,
};

static int mx6q_seco_q7_spi_3_cs_revC[] = {
        MX6_SECO_UQ7_ECSPI3_CS3,
};

static const struct spi_imx_master mx6q_seco_q7_spi_data_revA_B __initconst = {
	.chipselect     = mx6q_seco_q7_spi_cs_revA_B,
	.num_chipselect = ARRAY_SIZE(mx6q_seco_q7_spi_cs_revA_B),
};
...
static const struct spi_imx_master mx6q_seco_q7_spi_2_data_revC __initconst = {
        .chipselect     = mx6q_seco_q7_spi_2_cs_revC,
        .num_chipselect = ARRAY_SIZE(mx6q_seco_q7_spi_2_cs_revC),
};
...
static void tsc2006_dev_init(void) {
	...
	if (gpio_request(MX6_SECO_UQ7_PEN_IRQ, "TSC2006 pendown") < 0) {
		printk("can't get tsc2006 pen down GPIO\n");
	}

        gpio_direction_input(MX6_SECO_UQ7_PEN_IRQ);
	msleep(5);
}
...
static void gpio_expander_reset (void)
{
	int ret = 0;

        ret = gpio_request (MX6_SECO_UQ7_GPIO_EX_RST, "GPIO EXPANDER RESET");
        if (ret) {
                printk("failed to get MX6_SECO_UQ7_GPIO_EX_RST: %d\n", ret);
        } else {
		gpio_direction_output(MX6_SECO_UQ7_GPIO_EX_RST, 1);
                gpio_set_value(MX6_SECO_UQ7_GPIO_EX_RST, 1);
                printk("Resetting Gpio Expander.\n");
                gpio_set_value(MX6_SECO_UQ7_GPIO_EX_RST, 0);
                msleep(2);
                gpio_set_value(MX6_SECO_UQ7_GPIO_EX_RST, 1);
                msleep(1);
        }
...
static struct mcp23s08_platform_data mcp23s08_config __initdata = {
	.base = 224,
	.chip[0].is_present = false,
	.chip[1].is_present = false,
	.chip[2].is_present = false,
	.chip[3].is_present = false,
	.chip[4].is_present = false,
	.chip[5].is_present = false,
	.chip[6].is_present = false,		
	.chip[7].is_present = true,
	.reset	            = gpio_expander_reset,

        .irq_a              = {0, 0, 0, 0, 0, 0, 0, gpio_to_irq(MX6_SECO_UQ7_GPIO_INTA)}, 
        .irq_flags_a        = {0, 0, 0, 0, 0, 0, 0, IRQF_TRIGGER_FALLING},

        .irq_b              = {0, 0, 0, 0, 0, 0, 0, gpio_to_irq(MX6_SECO_UQ7_GPIO_INTB)},
        .irq_flags_b        = {0, 0, 0, 0, 0, 0, 0, IRQF_TRIGGER_FALLING},
	
};
#endif


static struct spi_board_info imx6_seco_q7_spi_device_revA_B[] __initdata = {
#if defined(CONFIG_Q7_MTD_M25P80)
	{
		.modalias = "m25p80",
...
#ifdef CONFIG_Q7_TOUCHSCREEN_TSC2006
        {
				...
				.irq 					= gpio_to_irq(MX6_SECO_UQ7_PEN_IRQ),
        },
#endif
};
...
#endif
#endif
#if defined(CONFIG_GPIO_MCP23S08)
	{
                .modalias               = "mcp23s17",
                .bus_num                = 1,
                .chip_select            = 1,
...
static enum ECTRL_EVENTS ectrl_event_list [5] __initdata = {
	EVNT_PWR_BTN,
	EVNT_RST_BTN,
	EVNT_SLEEP,
        EVNT_BATTERY,
...
static struct ectrl_bootdev bootdev_list [3] __initdata = {
	{
		.id     = BOOT_USBHC1,
		.label  = "external SD",
	}, {
...
static struct ectrl_platform_data mx6_seco_q7_ectrl = {
	...
	.irq			= gpio_to_irq(MX6_SECO_UQ7_ECTRL_IRQ),
...
static struct mxt_platform_data mxt_ts_platform_data = {

	.cfg_name               = "maxtouch.cfg",
	.irqflags = IRQF_TRIGGER_LOW |IRQF_ONESHOT,
	.gpio_reset = ATMEL_MXT_RST,
};

static void atmel_mxt_ts_init(void){
	
	int ret = 0;

	ret = gpio_request(ATMEL_MXT_RST, "Touch Atmel Reset");
        if (ret) {
                pr_err("failed to get GPIO ATMEL_MXT_RST,: %d\n",
                        ret);
                return;
        }
        gpio_direction_output(ATMEL_MXT_RST, 1);
	msleep(1);
	gpio_set_value(ATMEL_MXT_RST, 0);
	msleep(1);
	gpio_set_value(ATMEL_MXT_RST, 1);
...
static int tsc2004_get_pendown_state (void) {
        return gpio_get_value(MX6_SECO_UQ7_PEN_IRQ);
}
...
static struct i2c_board_info mxc_i2c2_board_info[] __initdata = {
...
#ifdef CONFIG_Q7_TOUCHSCREEN_ATMEL_MXT
    {
       ...
       .platform_data  = &mxt_ts_platform_data,
       .irq = gpio_to_irq(ATMEL_MXT_IRQ),
    },
#endif
};
...
static const struct flexcan_platform_data imx6_seco_uq7_flexcan0_pdata __initconst = {
...
static void __init mx6_seco_q7_board_init(void)
{
	...
	if (cpu_is_mx6q()){
		mxc_iomux_v3_setup_multiple_pads(mx6qd_seco_uq7_pads,
			ARRAY_SIZE(mx6qd_seco_uq7_pads));
		if (PCB_IS_REV_A || PCB_IS_REV_B)	
		mxc_iomux_v3_setup_multiple_pads(mx6qd_seco_uq7_pads_revA_B,
			ARRAY_SIZE(mx6qd_seco_uq7_pads_revA_B));
		if (PCB_IS_REV_C)
		...
		mxc_iomux_v3_setup_multiple_pads(mx6sdl_seco_uq7_pads,
			ARRAY_SIZE(mx6sdl_seco_uq7_pads));
	...
	if (ret) {
		...
		gpio_direction_input (MX6_SECO_UQ7_USB_OTG_ID);
			
	}
	gpio_free(MX6_SECO_UQ7_USB_OTG_ID);
	
	ret = gpio_request (MX6_SECO_UQ7_LVDS_BLT_CTRL, "LVDS backlight");
	if (ret) {
		printk("failed to get MX6_SECO_UQ7_LVDS_BLT_CTRL: %d\n", ret);
	} else {
		gpio_direction_output(MX6_SECO_UQ7_LVDS_BLT_CTRL, 1);
	}
	
	ret = gpio_request (MX6_SECO_UQ7_LVDS_PPEN_CTRL, "PPEN backlight");
        if (ret) {
                printk("failed to get MX6_SECO_UQ7_LVDS_PPEN_CTRL: %d\n", ret);
        } else {
                gpio_direction_output(MX6_SECO_UQ7_LVDS_PPEN_CTRL, 1);
        }

	gpio_set_usbotg_init();
...
//   GPIO for Ethernet reset
	gpio_request(MX6_SECO_UQ7_FEC_RESET, "fec-reset");
	gpio_direction_output(MX6_SECO_UQ7_FEC_RESET, 1);
//   GPIO for Ethernet reset
 
	gp_reg_id = seco_q7_dvfscore_data.reg_id;
	soc_reg_id = seco_q7_dvfscore_data.soc_id;
	pu_reg_id = seco_q7_dvfscore_data.pu_id;
	mx6q_seco_uq7_init_uart();
	...
	ret = gpio_request(MX6_SECO_UQ7_PMIC_INT, "pFUZE-int");
	if (ret) {
		printk(KERN_ERR "request pFUZE-int error!!\n");
		return;
	} else {	
		gpio_direction_input(MX6_SECO_UQ7_PMIC_INT);
		mx6q_seco_q7_init_pfuze100(MX6_SECO_UQ7_PMIC_INT);
	}
...
#if defined(CONFIG_UQ7_EMMC_INTERFACE) // eMMC
	imx6q_add_sdhci_usdhc_imx(2, &mx6q_seco_q7_sd3_data);
#endif
#if defined(CONFIG_UQ7_SD_INTERFACE) // SD external Card on CROSSPLATFORM
	imx6q_add_sdhci_usdhc_imx(3, &mx6q_seco_q7_sd4_data);
#endif
//#if defined(CONFIG_UQ7_SD2_INTERFACE)  // none
...
//#endif
	...
	ret = gpio_request (MX6_SECO_UQ7_CAN_EN, "CAN EN");
        if (ret) {
                printk("failed to get MX6_SECO_UQ7_CAN EN: %d\n", ret);
        } else {
#if defined(CONFIG_Q7_CAN)
                gpio_direction_output(MX6_SECO_UQ7_CAN_EN, 0);
#else
                gpio_direction_output(MX6_SECO_UQ7_CAN_EN, 1);
#endif
        }
#if defined(CONFIG_Q7_CAN)
        imx6q_add_flexcan0(&imx6_seco_uq7_flexcan0_pdata);
#endif
	...
	gpio_set_value(MX6_SECO_UQ7_USB_HUB_RESET, 1);
...
#ifdef CONFIG_Q7_PWM3
	imx6q_add_mxc_pwm_generic (1, &mx6_seco_q7_pwm3_generic_data);
#endif
#ifdef CONFIG_Q7_PWM2
 ...
 */
MACHINE_START(MX6_SECO_UQ7, "SECO i.MX6 Quad/Dual/Dual Lite/Solo uQ7 Board")
