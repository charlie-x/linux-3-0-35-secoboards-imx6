...
#define UART_RS485_WAIT		msecs_to_jiffies(1*1000)       // 10s
...
static int serial_imx_probe(struct platform_device *pdev)
{
	...
	if (pdata && (pdata->flags & IMXUART_SDMA))
		sport->enable_dma = 1;
	if (pdata && (pdata->flags & IMXUART_IS_RS485)) {
		printk(KERN_INFO "Serial: enabled RS485 mode\n");
		sport->enable_rs485 = 1;
		sport->port.rs485.flags = SER_RS485_ENABLED;// | SER_RS485_RTS_ON_SEND;
		imx_rs485_config(&(sport->port));
		if (pdata->rts_gpio > 0) {
			sport->rts_g = pdata->rts_gpio;
